...
export const PLAYER_SKINS = [
  { id: 'ochre', name: 'Male · Ochre traveler' },
  { id: 'sage', name: 'Male · Sage traveler' },
  { id: 'indigo', name: 'Male · Indigo elder' },
  { id: 'terracotta', name: 'Female · Terracotta traveler' },
  { id: 'plum', name: 'Female · Plum traveler' },
  { id: 'teal', name: 'Female · Teal elder' }
];
export const PLAYER_PALETTE = ['#bb6249', '#428b90', '#bc9236', '#8870b0'];
export const PLAYER_ATLAS = 'assets/rebirth/travelers.png';
export const skinIndex = skin => Math.max(0, PLAYER_SKINS.findIndex(s => s.id === skin));
export function paintPlayer(el, player) {
  const n = skinIndex(player.skin);
  el.classList.add('player-art');
  el.style.setProperty('--skin-x', (n % 3) * 50 + '%');
  el.style.setProperty('--skin-y', Math.floor(n / 3) * 100 + '%');
...
export function createPlayerArt(THREE, tokens, size, invalidate) {
  ...
  function sync(players) {
    ...
    players.forEach((p, i) => {
      ...
      if (sprites[i]) sprites[i].material.map = maps[skinIndex(p.skin)];
    });
  }
  function load(players) {
    ...
    new THREE.TextureLoader().load(PLAYER_ATLAS, texture => {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        map.offset.set((i % 3) / 3, 1 / 2 - Math.floor(i / 3) / 2);
        map.needsUpdate = true;
        maps.push(map);
      }
      tokens.forEach((token, i) => {
        const material = new THREE.SpriteMaterial({ map: maps[0], transparent: true, alphaTest: .08, depthWrite: false });
        const sprite = new THREE.Sprite(material);
        sprite.scale.set(size * .42, size * .63, 1);
        sprite.position.y = size * .30;
        token.add(sprite);
